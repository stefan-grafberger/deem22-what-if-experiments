...
def execute_image_pipeline_w_shapley_naive(corrupted_row_ids: pd.DataFrame, label_corrections: pd.DataFrame,
    ...
    train_data = pd.read_csv(f'{str(get_project_root())}/whatif/experiments/datasets/sneakers/'
                             f'product_images_corrupted_train.csv',
                             converters={'image': decode_image})
    enable_fact_table_row_tracking_naive(train_data)

    test_data = pd.read_csv(f'{str(get_project_root())}/whatif/experiments/datasets/sneakers/'
                            f'product_images_test.csv',
                            converters={'image': decode_image})

    train_data = apply_label_corrections_to_train_data_naive(label_corrections, train_data)

    product_categories = pd.read_csv(
        f'{str(get_project_root())}/whatif/experiments/datasets/sneakers/product_categories.csv')
    ...
    train_with_categories = train_data.merge(product_categories, on='category_id')
    test_with_categories = test_data.merge(product_categories, on='category_id')

    categories_to_distinguish = ['Sneaker', 'Ankle boot']

    train_images_of_interest = train_with_categories[
        train_with_categories['category_name'].isin(categories_to_distinguish)]
    test_images_of_interest = test_with_categories[
        test_with_categories['category_name'].isin(categories_to_distinguish)]
    ...
    y_train = label_binarize(train_images_of_interest['category_name'], classes=categories_to_distinguish)
    y_test = label_binarize(test_images_of_interest['category_name'], classes=categories_to_distinguish)

    image_lineage_ids = save_row_tracking_information_naive(train_images_of_interest)

    x_train = pipeline_without_model.fit_transform(
        train_images_of_interest[['image']])  # beide varianten mal ausprobieren, unklar was fair
    model_without_pipeline.fit(x_train, y_train)

    x_test = pipeline_without_model.transform(test_images_of_interest[['image']])
    ...
    cleaning_results = do_shapley_value_cleaning_naive(corrupted_row_ids, image_lineage_ids, label_corrections,
                                                       shapley_value_cleaning, total_updates, train_images_of_interest,
                                                       x_test, x_train, y_test, y_train,
                                                       shapley_value_k, cleaning_batch_size)
...
def create_corrupt_train_test_data(corruption_fraction=0.5):
    # TODO change this to pyarrow + parquet, which can handle numpy arrays well
    all_data = pd.read_csv(
        f'{str(get_project_root())}/whatif/experiments/datasets/sneakers/product_images.csv')
    ...
    test_data = test_data[['image', 'category_id']]
    test_data.to_csv(f'{str(get_project_root())}/whatif/experiments/datasets/'
                     f'sneakers/product_images_test.csv', index=False)
    ...
    corrupted_df_to_save.to_csv(f'{str(get_project_root())}/whatif/experiments/datasets/'
                                f'sneakers/product_images_corrupted_train.csv', index=False)

    corrupted_row_ids = rows_to_corrupt[['image_lineage_id']]
    corrupted_row_ids = corrupted_row_ids.sort_index()
    corrupted_row_ids.to_csv(f'{str(get_project_root())}/whatif/experiments/datasets/'
                             f'sneakers/product_image_ids_corrupted_train.csv', index=False)
    return corrupted_row_ids


def do_shapley_value_qualitative(corruption_fraction, num_iterations, use_shapley_weighting, shapley_value_k,
                                 cleaning_batch_size):
    corrupted_row_ids = create_corrupt_train_test_data(corruption_fraction)
