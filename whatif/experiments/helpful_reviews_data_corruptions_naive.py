...
def execute_review_pipeline_naive(corrupt_train, corrupt_test, corruption_fraction, corrupt_feature, debug):
    ...
    reviews = pd.read_csv(f'{str(get_project_root())}/whatif/experiments/datasets/amazon-reviews/reviews.csv.gz',
                          compression='gzip', index_col=0)
    products = pd.read_csv(f'{str(get_project_root())}/whatif/experiments/datasets/amazon-reviews/products.csv',
                           index_col=0)
    categories = pd.read_csv(
        f'{str(get_project_root())}/whatif/experiments/datasets/amazon-reviews/categories.csv', index_col=0)
    ratings = pd.read_csv(f'{str(get_project_root())}/whatif/experiments/datasets/amazon-reviews/ratings.csv',
                          index_col=0)
...
def measure_review_corruption_naive_exec_time(debug, corruption_percentages, corrupt_features, repeats=10):
    ...
    """),
                           setup=cleandoc(f"""
    from whatif.experiments.helpful_reviews_data_corruptions_naive import execute_review_pipeline_naive
    """),
