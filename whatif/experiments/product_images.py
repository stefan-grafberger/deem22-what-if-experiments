...
def execute_image_pipeline():
    ...
    train_data = pd.read_csv( f'{str(get_project_root())}/whatif/experiments/datasets/sneakers/product_images.csv', converters={'image': decode_image})

    product_categories = pd.read_csv( f'{str(get_project_root())}/whatif/experiments/datasets/sneakers/product_categories.csv')
